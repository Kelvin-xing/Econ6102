...
for earning_report in qa_all:
    ...
    for qa in earning_report["QA_session"]:
        ...
        for session in qa:
            ...
            if result == [0]:
                continue
            record_temp = [temp_company, temp_year, temp_quarter, temp_Q_company, temp_Q_analyst,
                           str(session_number)]
            ...
            temp_new_result = "\t".join(record_temp) + "\t" + str(mean_cor)[0:4] + "\t" + str(
                N_A_paragraphs) + "\t" + str(subsession_number) + "\t" + temp_manager + "\t" + str(
                n_sentence) + "\t" + str(n_cor) + "\t" + str(sen_cor) + "\t" + str(eff_q_words) + "\t" + str(
                eff_a_words) + "\n"
            file_str = file_str + temp_new_result.encode("utf-8")
            cor_table.append([temp_Q_analyst, temp_Q_company, mean_cor])
            # print("total file is\n")
            # print(file_str)
# print(type(file_str))
# print(len(file_str))
...
cor_table.to_csv("qa_similarity_analysis.csv", index=False)



#Now we get a sentiment analysis table,next we get missing value



# Load JSON data from file
with open('qa_list_all.txt', 'r', encoding='utf-8') as file:
    data = json.load(file)

# Initialize results list and counters for total and missing entries
results = []
total_data_count = 0
missing_data_count = 0

for record in data:
    for session in record["QA_session"]:
        for qa in session:
            analyst = qa.get("Q_analyst_name", "").strip()
            firm = qa.get("Q_company_name", "").strip()
            total_data_count += 1

            if not analyst or not firm:
                missing_data_count += 1
                continue

missing_percentage = (missing_data_count / total_data_count) * 100

# Output summary of missing data
